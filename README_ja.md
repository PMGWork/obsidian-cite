...
> Cite 0.1.0にはObsidian 1.12.7以降が必要です。デスクトップ、iOS、Androidに対応します。
...
リリース前の全チェックは `npm run check` で実行できます。production版の `main.js` はローカルで生成してGitHub Releaseへ添付し、リポジトリにはコミットしません。
